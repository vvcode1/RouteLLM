...
This script parses MBOX format email archives and converts them to CSV format.
MBOX files contain email messages in a standard Unix mailbox format.

Prerequisites:
1. Download the MBOX file from your email archive source
2. Place the MBOX file at the specified path: ../data/itnog/itnog.mbox
...
# Define MBOX file path
mbox_file_path = "../data/itnog/itnog.mbox"
# Define output CSV path
output_csv_path = "../data/itnog/itnog_email.csv"
...
def parse_mbox_to_csv(mbox_path, output_path):
    """
    Parse MBOX file and convert to CSV format.
    ...
    if not os.path.exists(mbox_path):
        print(f"Error: MBOX file not found at {os.path.abspath(mbox_path)}")
        print("Please download the MBOX file and place it at the specified path.")
        print("Instructions:")
        print("1. Download the MBOX archive from your email provider or mailing list")
        print("2. Save it as 'itnog.mbox' in the '../data/itnog/' directory")
    ...
    with open(mbox_path, "rb") as mbox_file, open(output_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        print(f"Processing MBOX file: {os.path.abspath(mbox_path)}")
...
if __name__ == "__main__":
    print("MBOX Email Parser")
    print("=" * 50)
    print("This script converts MBOX format email archives to CSV.")
    print()
