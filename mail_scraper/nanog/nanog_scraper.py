"""
NANOG MBOX Email Parser

This script parses NANOG mailing list MBOX format email archives and converts them to CSV format.
MBOX files contain email messages in a standard Unix mailbox format.

Prerequisites:
1. Download the NANOG MBOX file from the mailing list archive
2. Place the MBOX file at the specified path: ../data/nanog/nanog@lists.nanog.org.mbox
3. Run this script to convert emails to CSV format

Usage:
    python nanog_mbox_parser.py
...
# Define NANOG MBOX file path
mbox_file_path = "../data/nanog/nanog@lists.nanog.org.mbox"
# Define output CSV path
output_csv_path = "../data/nanog/nanog_email.csv"
...
def parse_mbox_to_csv(mbox_path, output_path):
    ...
    with open(mbox_path, "rb") as mbox_file, open(output_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        print(f"Processing NANOG MBOX file: {os.path.abspath(mbox_path)}")
...
if __name__ == "__main__":
    print("NANOG MBOX Email Parser")
    print("=" * 50)
    print("This script converts NANOG mailing list MBOX archives to CSV.")
    print()
